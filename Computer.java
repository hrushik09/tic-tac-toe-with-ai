...
public class Computer extends Player {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (game.grid[i][j] == 32) {
                    ...
                    game.grid[i][j] = 32;
                    if (moveVal > bestVal) {
                        firstCoordinate = (char) (i + 48 + 1);
                        secondCoordinate = (char) (j + 48 + 1);
                        bestVal = moveVal;
                    }
                }
            }
    ...
    private boolean getRandomCoordinates(Game game) {
        ...
        do {
            firstCoordinate = (char) (random.nextInt(3) + 48 + 1);
            secondCoordinate = (char) (random.nextInt(3) + 48 + 1);
        } while (game.grid[firstCoordinate - 1 - 48][secondCoordinate - 1 - 48] != 32);
        return makeMove(game);
    }
    ...
    private boolean isEmpty(Game game, int p, int q) {
        if (game.grid[p][q] == 32) {
            firstCoordinate = (char) (p + 48 + 1);
            secondCoordinate = (char) (q + 48 + 1);
            return true;
        }
        return false;
    }
